...
vi.mock('@/lib/api-utils', () => ({
  ...
  CacheKeys: {
    ...
    subscriptions: (params: any) => 'subscriptions:' + JSON.stringify(params),
  },
...
describe('API /subscriptions', () => {
  ...
  describe('GET /api/subscriptions', () => {
    ...
    it('should create missing subscriptions when ensureSubsExist is called', async () => {
      vi.mocked(db.client.findMany).mockResolvedValue([{ id: 'client-1' }] as any)
      vi.mocked(db.subscription.findMany).mockResolvedValue([])
      vi.mocked(db.settings.findUnique).mockResolvedValue(null)
      vi.mocked(db.subscription.createMany as any).mockResolvedValue({ count: 1 })
