...
async function ensureSubscriptionsExist(month: number, year: number) {
  // BOLT OPTIMIZATION: Use 'none' filter to find missing clients in a single query (O(1) database trip)
  ...
  if (missingClients.length > 0) {
    const missingIds = missingClients.map(c => c.id)

    // BOLT OPTIMIZATION: Parallelize settings fetch and previous month data fetch
    ...
    const [defaultClassesSetting, prevSubscriptions] = await Promise.all([
      ...
      // BOLT OPTIMIZATION: Fetch only relevant previous month subscriptions for missing clients
    ...
    // BOLT OPTIMIZATION: Use createMany for bulk insertion (O(1) database trip instead of O(N))
    const dataToInsert = missingClients.map(client => {
