...
async function ensureSubscriptionsExist(month: number, year: number) {
  // BOLT OPTIMIZATION: Use 'none' filter to find clients without subscriptions for this month/year in a single query
  // This avoids fetching all clients and all subscriptions to filter in-memory (O(N) -> O(1) database roundtrips)
  ...
  if (missingClients.length > 0) {
    ...
    // This reduces database roundtrips from O(M) to O(1) where M is the number of missing clients
