...
async function ensureSubscriptionsExist(month: number, year: number) {
  // BOLT OPTIMIZATION: Use 'none' filter to find clients without subscriptions in ONE query
  // This reduces database roundtrips and avoids fetching all clients into memory.
  ...
  if (missingClients.length > 0) {
    ...
    // BOLT OPTIMIZATION: Use createMany for bulk insertion (1 query vs N queries)
