...
async function ensureSubscriptionsExist(month: number, year: number) {
  // BOLT OPTIMIZATION: Use 'none' filter to find clients missing subscriptions for the month/year in a single query.
  // This replaces fetching all clients and all existing subscriptions separately (2 queries + JS filtering).
  ...
  if (missingClients.length > 0) {
    ...
    // This is significantly more efficient for large numbers of records.
