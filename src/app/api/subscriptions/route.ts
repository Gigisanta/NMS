...
async function ensureSubscriptionsExist(month: number, year: number) {
  // BOLT OPTIMIZATION: Use 'none' filter to find clients missing subscriptions in a single query
  // instead of fetching all clients and subscriptions and filtering in-memory (O(N+M) -> O(1) queries)
  ...
  if (missingClients.length > 0) {
    ...
    // BOLT OPTIMIZATION: Use createMany for O(1) bulk insertion instead of O(N) individual creates
    // This significantly reduces database roundtrips for large client lists.
