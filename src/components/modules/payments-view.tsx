...
// BOLT OPTIMIZATION: Memoized table row to skip redundant renders
const PaymentTableRow = memo(({ sub, index, isUpdating, onPaymentClick, onStatusChange }: {
  sub: Subscription
  index: number
  isUpdating: boolean
...
export function PaymentsView() {
  ...
  // BOLT OPTIMIZATION: Memoize handlers to keep child props stable
  ...
  const { filteredSubscriptions, stats } = useMemo(() => {
    const stats = { total: subscriptions.length, alDia: 0, pendiente: 0, deudor: 0 }
    const filtered = []
