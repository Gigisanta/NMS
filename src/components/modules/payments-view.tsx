...
// BOLT: Static array moved outside to prevent re-creation on every render
const months = Array.from({ length: 12 }, (_, i) => i + 1)

// BOLT OPTIMIZATION: Extract row into a memoized component to prevent
// unnecessary re-renders of all rows when only one is updating.
const SubscriptionRow = memo(({
  ...
  const isLate = isLateDay && sub.status === 'PENDIENTE'
...
export function PaymentsView() {
  ...
  const handleStatusChange = useCallback(async (subscriptionId: string, newStatus: string, paymentMethod?: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // @ts-ignore - Zustand state might not be perfectly typed in all contexts
  ...
  // This reduces the number of array iterations from 5 to 1.
  const { filteredSubscriptions, stats } = useMemo(() => {
    const statsObj = { total: 0, alDia: 0, pendiente: 0, deudor: 0 }
    const filtered = []

    for (const s of subscriptions) {
      // Calculate stats for all subscriptions in the current period
      statsObj.total++
      if (s.status === 'AL_DIA') statsObj.alDia++
      else if (s.status === 'PENDIENTE') statsObj.pendiente++
      else if (s.status === 'DEUDOR') statsObj.deudor++
  ...
  // BOLT: Pre-calculate isLateDay once instead of every row
  const isLateDay = useMemo(() => new Date().getDate() > 10, [])
