...
// BOLT OPTIMIZATION: Extracting Row component to React.memo to prevent re-renders of the entire list
// when only one row's 'updating' state changes or when filtering.
const SubscriptionRow = memo(({
  ...
  index: number,
  isUpdating: boolean,
  handlePaymentClick: (sub: Subscription, method: string) => void,
  handleStatusChange: (id: string, status: string, method?: string) => void
}) => {
...
export function PaymentsView() {
  ...
  const handleStatusChange = useCallback(async (subscriptionId: string, newStatus: string, paymentMethod?: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // @ts-ignore - invalidateDashboard is optional in the store
  ...
  // BOLT OPTIMIZATION: Combine filtering and statistics calculation into a single O(N) pass.
  // This avoids iterating through the entire subscriptions array 4+ times on every render.
  const { filteredSubscriptions, stats } = useMemo(() => {
    ...
    for (const sub of subscriptions) {
      // Update statistics (on the whole set)
      if (sub.status === 'AL_DIA') statistics.alDia++
      else if (sub.status === 'PENDIENTE') statistics.pendiente++
      else if (sub.status === 'DEUDOR') statistics.deudor++
    ...
    return { filteredSubscriptions: filtered, stats: statistics }
