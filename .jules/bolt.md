...
## 2025-05-15 - Efficient Existence Checks and Bulk Insertion
**Learning:** Finding missing related records by fetching all parent and child records to compare in JavaScript is an O(N) memory and bandwidth bottleneck. Prisma's `none` relation filter performs this check efficiently on the database side. Combined with `createMany` for the results, it reduces the entire operation to O(1) database trips.
**Action:** Use `where: { relation: { none: { ... } } }` to identify missing records and `createMany` for bulk initialization.
