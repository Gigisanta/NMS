...
## 2026-03-04 - Efficient Record Generation with Prisma
**Learning:** Checking for record existence and creating missing ones often leads to O(N) database roundtrips if done in a loop. Prisma's `none` filter allows identifying all missing records in a single `findMany` query, and `createMany` handles the bulk insertion in one roundtrip.
**Action:** Use `where: { relation: { none: { ... } } }` to find missing related records and `createMany({ data, skipDuplicates: true })` for efficient generation. This reduces total database roundtrips from O(N) to O(1) for auto-generation logic.
