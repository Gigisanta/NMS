...
## 2026-05-17 - Optimizing Bulk Record Generation
**Learning:** Checking for missing records by fetching all parent and child records to compare in JavaScript is an O(N) bottleneck. Prisma's `none` relation filter allows the database to identify missing records in a single query. Furthermore, wrapping individual `create` calls in a transaction is significantly slower than using `createMany` for bulk insertion.
**Action:** Use Prisma's `none` filter to identify missing records in one-to-many relationships and `createMany` for efficient bulk creation.
