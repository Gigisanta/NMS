...
## 2026-03-04 - useCallback and Stale Closures in API Handlers
**Learning:** Wrapping API interaction handlers in `useCallback` with empty dependencies can lead to stale closures if the handler depends on current render state (like month/year filters).
**Action:** Either include all state dependencies in the `useCallback` array or use functional state updates and direct store access (`useAppStore.getState()`) to keep the handler stable while ensuring it uses the latest data.
