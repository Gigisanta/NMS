...
## 2026-03-04 - Efficient Bulk Record Generation
**Learning:** Generating missing records for a large set (e.g., monthly subscriptions for all clients) using manual filtering in JS and individual `create` calls within a transaction is extremely slow and database-intensive.
**Action:** Use Prisma's `none` relationship filter to identify missing records in a single query, and `createMany({ skipDuplicates: true })` for efficient bulk insertion. This reduces database roundtrips from O(N) to O(1).
