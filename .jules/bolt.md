...
## 2026-03-04 - Batching Subscription Generation
**Learning:** Automatically generating missing records (e.g., monthly subscriptions) on access is a common pattern, but doing so with individual queries (find all clients, find existing, filter, then create in loop) scales poorly ((N)$).
**Action:** Use Prisma's `none` relation filter to identify missing records in a single database query ((1)$) and use `createMany` for bulk insertion to minimize database roundtrips.
